describe('The PACER module', function () {
  ...
  describe('getCaseIdFromAppellateDocketPage', () => {
    it('returns the caseId', () => {
      const anchor = document.createElement('a');
      anchor.setAttribute('title', 'Open Document');
      anchor.setAttribute('onclick', 'return doDocPostURL(\'00107526453\', \'45846\' )');
      const result = PACER.getCaseIdFromAppellateDocketPage([anchor]);
      expect(result).toEqual('45846');
    });
  });

  describe('getCaseIdFromAppellateCaseQuery', () => {
    // only return lower [i] caseid -- i.e., caseId should not match
    it('returns the caseid', () => {
      const input = document.createElement('input');
      input.setAttribute('name', 'caseid');
      input.value = '45846';
      const result = PACER.getCaseIdFromAppellateCaseQueryPage([input]);
      expect(result).toEqual('45846');
    });
  });

  describe('getCaseIdFromAppellateSearchResults', () => {
    it('returns the caseid', () => {
      const anchor = document.createElement('a');
      anchor.href = 'https://ecf.ca9.uscourts.gov/n/beam/servlet/TransportRoom?servlet=CaseQuery.jsp&cnthd=4176531425&caseid=302206&csnum1=18-35460&shorttitle=Edwin+Omar+Flores+Tejada%2C+et+al+v.+Elizabeth+Godfrey%2C+et+al'
      const result = PACER.getCaseIdFromAppellateSearchResults([anchor]);
      expect(result).toEqual('302206');
    });
  });
