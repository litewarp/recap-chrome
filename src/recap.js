...
function Recap() {
  ...
  return {
    ...
    getAvailabilityForDocket: function (pacer_court, pacer_case_id, cb) {
      ...
      console.info(
        `RECAP: Getting availability of docket ${pacer_case_id} at ` +
          `${pacer_court}`
      );
      $.ajax({
        ...
        error: function (xhr, textStatus, errorThrown) {
          console.error(
            `RECAP: Ajax error getting docket availability. Status: ` +
    ...
    uploadDocket: function (pacer_court, pacer_case_id, html, upload_type, cb) {
      ...
      $.ajax({
        ...
        success: function (data, textStatus, xhr) {
          console.info(
            `RECAP: Successfully uploaded docket or docket ` +
        ...
        error: function (xhr, textStatus, errorThrown) {
          console.error(
            `RECAP: Ajax error uploading docket. Status: ${textStatus}.` +
              `Error: ${errorThrown}`
          );
        },
      });
    ...
    uploadAttachmentMenu: function (pacer_court, pacer_case_id, html, cb) {
      ...
      $.ajax({
        ...
        success: function (data, textStatus, xhr) {
          console.info(
            `RECAP: Successfully uploaded attachment page: '${textStatus}' ` +
    ...
    uploadDocument: (
      ...
      getItemsFromStorage(cb.tab.id)
        .then(async (tabStorage) => {
          // create form data
          const blob = await fetch(tabStorage['pdf_blob']).then((res) =>
            res.blob()
          );
          let formData = new FormData();
          formData.append(
            'court',
            PACER.convertToCourtListenerCourt(pacer_court)
          );
          pacer_case_id && formData.append('pacer_case_id', pacer_case_id);
